...
def test_recruit():
    ...
    print('初始化是否为空：', test_recruit.is_empty())
    print('此时的 size ：', test_recruit.get_size())
    print('将 record 中的字符串全部添加进 test：')
    for elem in record:
        test_recruit.add_recruit(test_recruit._root, elem, 0)
    print('判断 record 中元素是否都已经存在于 test 中：')
    for elem in record[::-1]:
        flag = test_recruit.contains_recruit(test_recruit._root, elem, 0)
        if flag:
            print('"%s" 存在于 test 中' % elem)
    print("此时的 test 的 size：", test_recruit.get_size())
    print('"hel" 是否是 test 的前缀：', test_recruit.is_prefix('hel'))
